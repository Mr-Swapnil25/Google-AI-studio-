...
async function fetchFromDataGov(
  ...
  try {
    const response = await fetch(`${DATA_GOV_BASE_URL}?${params.toString()}`);
    
    if (!response.ok) {
      throw new Error(`API returned ${response.status}: ${response.statusText}`);
    }

    const data = await response.json() as { records?: DataGovRecord[] };
    return data.records || [];
  } catch (error) {
 ...
 */
function buildDocId(state: string, district: string, commodity: string): string {
 ...
 */
async function processCommodityPrices(commodity: string): Promise<number> {
 ...
 */
export const fetchMandiPrices = functions
  .runWith({
 ...
 */
export const fetchMandiPricesHttp = functions
  .runWith({
    timeoutSeconds: 540,
    memory: '512MB',
  })
  ...
  .onRequest(async (req, res) => {
    ...
    const filterCommodity = req.query.commodity as string;
 ...
 */
export const syncMandiPricesNow = functions
  .runWith({
    timeoutSeconds: 300,
    memory: '256MB',
  })
